...
- **Layout system:** Recursive tree of PanelNodes/GroupNodes. Base 5 panel types (`BasePanelId`: chat, files, editor, preview, terminal) + dynamic detached editors (`PanelId = BasePanelId | 'editor:${string}'`). DnD with @dnd-kit, resizable via react-resizable-panels. `PanelContent` uses `getPanelIcon()`/`getPanelTitle()` functions (not static Records) to support dynamic panel IDs. `DroppablePanel` handles close button differently for detached editors (`closeDetachedEditor` + remove from tree, not reattach). DnD state: `draggedPanelId` for panels, `draggedEditorTabId` for FM tab drags. `isFirst` prop propagated through `LayoutRenderer` → `DroppablePanel` to show `SidebarToggleBtn` in the first panel's header. Panel headers have context menu (right-click desktop / long-press mobile): Hide, Open in Separate Panel, Reset Layout
...
- **Terminal:** Cross-platform PTY via `aymanbagabas/go-pty` (Unix PTY + Windows ConPTY). Backend: `defaultShell()` resolves absolute path via `exec.LookPath` (MUST be absolute — go-pty resolves relative to `cmd.Dir` on Windows). `TerminalSession` struct holds `Pty gopty.Pty`, `Cmd *gopty.Cmd`, `Done chan`. Frontend: module-level singleton separates xterm (`session: TermSession`) from WebSocket (`currentWs: WebSocket | null`) — xterm is created once and lives forever, WebSocket reconnects independently (up to 5 attempts, 800ms delay). `session` holds xterm + FitAddon + SearchAddon. Component attaches/detaches xterm DOM node via `el.appendChild(xtermEl)`. Context menu: Copy, Paste, Select All, Find (SearchAddon), Font+/Font−, Clear, Clear Console, Reconnect. Font size persisted in localStorage (`clauder-terminal-font-size`, 8–24px). WebSocket binary for I/O, JSON for resize. ResizeObserver auto-fits on panel resize
