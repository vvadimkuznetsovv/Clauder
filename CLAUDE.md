# Nebulide — Web IDE for Claude Code
...
backend/
  ...
  handlers/
    ...
    proxy.go           # code-server reverse proxy (strips /code prefix, removes token param)
...
frontend/src/
  ...
  store/
    ...
    workspaceStore.ts  # Sessions, editor tabs (incl. tempTabId), file tree, preview, sidebarOpen
    layoutStore.ts     # Panel visibility toggles, activeIndex management on show/hide
  components/
    layout/            # Sidebar (Settings + panel toggles), LayoutRenderer, PanelContent, DnD panels
    chat/              # ChatPanel — code-server iframe proxy (probes /code/, shows iframe or error)
    editor/
    ...
    files/
      ...
      FileTreeItem.tsx # Single file/folder row: depth indentation, chevron (expand/collapse), icon/color by extension, inline rename. Folders have no icon (empty string in getIcon)
      ContextMenu.tsx  # Reusable portal-based context menu. Close-on-outside-click uses capture-phase pointerdown (not mousedown) — works inside Monaco/iframes that stopPropagation
    terminal/          # Terminal (xterm.js + WebSocket PTY, SearchAddon, font size persistence)
    preview/
  ...
  hooks/               # useAuth, useChat (50ms stream batching), useWebSocket, useLongPress (pointer events)
```
...
- **code-server integration:** `ChatPanel` repurposed as iframe proxy — probes `HEAD /code/?token=` (5s timeout), shows loading/error/iframe states. Backend: `/code/*path` route group with `AuthRequired` middleware → `handlers.CodeServerProxy()` reverse-proxies to `http://code-server:8443`, strips `/code` prefix, removes `token` param. Docker: `code-server` service in compose with `Dockerfile.code-server`. Vite proxy: `/code → http://localhost:8080` with `ws: true`
- **Layout system:** Recursive tree of PanelNodes/GroupNodes. Base 5 panel types (`BasePanelId`: chat, files, editor, preview, terminal) + dynamic detached editors (`PanelId = BasePanelId | 'editor:${string}' | 'terminal:${string}'`). DnD with @dnd-kit, resizable via react-resizable-panels. `PanelContent` uses `getPanelIcon()`/`getPanelTitle()` functions (not static Records) to support dynamic panel IDs. `DroppablePanel` handles close differently for detached panels: editors → `closeDetachedEditor` + remove; terminals → `destroyTerminalSession` + remove (via `toggleVisibility` or `removeDetachedPanel`). DnD state: `draggedPanelId` for panels, `draggedEditorTabId` for FM tab drags. `isFirst` prop propagated through `LayoutRenderer` → `DroppablePanel` to show `SidebarToggleBtn` in the first panel's header. Panel headers have context menu (right-click desktop / long-press mobile): Hide, Open in Separate Panel, Reset Layout
...
- **Terminal:** Cross-platform PTY via `aymanbagabas/go-pty` (Unix PTY + Windows ConPTY). Backend: `defaultShell()` resolves absolute path via `exec.LookPath` (MUST be absolute). `TerminalSession` has `GetOrCreate` (reuses alive shell), `Attach(conn)` (closes old WS, attaches new). Session key: `term:${userID}:${instanceId}` (`instanceId` from `?instanceId=` query param, default `"default"`). Frontend: **multi-instance** — module-level `Map<instanceId, TermSession>`. Each instance has own xterm + WS + reconnect state. `TermSession` stores `ws` as field (onData closes over it). Base `terminal` panel: `instanceId="default"`, `persistent=true` (xterm survives hide/show). Detached terminals `terminal:${instanceId}`: destroyed on panel close (`destroyTerminalSession`). Sidebar terminal button → `openNewTerminal()` (always opens new). Context menu: Copy, Paste, Select All, Find (SearchAddon), Font+/Font−, Clear, Clear Console, Reconnect. Font size persisted in localStorage (`nebulide-terminal-font-size`, 8–24px). WS binary for I/O, JSON for resize. ResizeObserver auto-fits on panel resize
- **Cross-OS:** `config.go` auto-detects Windows/Linux via `runtime.GOOS`. On Windows: `findProjectRoot()` walks up from `os.Executable()` dir and `os.Getwd()` looking for `.env` file — returns first match as project root → `<root>/workspace`. Fallback: `%USERPROFILE%\workspace`. `files.go` falls back to `ClaudeWorkingDir` when session path doesn't exist on current OS. `.env` must NOT set `CLAUDE_WORKING_DIR` for auto-detect to work
...
- **Mobile performance:** `@media (max-width: 640px)` kills expensive effects: hides blobs 5-12 (4 large 260-340px corner blobs remain, drift via existing `lava1-4` animations), reduces `backdrop-filter` from 40-50px to 6-12px, disables SVG `glass-distortion` filter (`feTurbulence`/`feDisplacementMap`) on panels, removes `glassShimmer` animation from close buttons/selected files/scrollbar (replaced with static neon purple). `body { animation: none }` kills `scrollbarShimmer` on mobile.
- **Tab close buttons:** Active tab shows close (X) button always; inactive tabs have close button with class `.panel-tab-close-compact` (hidden by default via `display: none`). Close buttons are Liquid Glass styled: purple-tinted background, `backdrop-filter: blur(10px)`, `glassShimmer` animation (purple↔white border/glow cycle, 3s). Sizes: panel tabs 20×20 (24 on touch), editor tabs 22×22 (26 on touch). SVG icon 12×12. Middle-click close only works on active editor tab. CSS classes: `.panel-tab-close`, `.panel-tab-close-compact`, `.tab-close-btn`. Animation stops on hover (replaced with brighter static glow). On mobile: static neon purple, no animation
...
- **Detached editors:** Users can drag editor tabs out of File Manager (or right-click → "Open in Separate Panel") to create standalone code editor panels in the layout. `PanelId` format: `editor:${tabId}`. State: `workspaceStore.detachedEditors` (Record<tabId, {filePath, modified}>). Layout actions in `layoutStore`: `detachEditorTab` (context menu, places right of FM), `detachEditorTabToSplit/ToEdge/ToMerge` (DnD drop handlers). `reattachEditor(panelId)` returns file to FM tabs. Close (X) on detached panel = `closeDetachedEditor` (destroys, not reattach). `toggleVisibility` for detached editors = `closeDetachedEditor` + remove from tree. EditorTabButton uses `useDraggable({ id: 'editor-tab:${tabId}' })` with `attributes` excluded (see overflow gotcha). Workspace.tsx `handleDragEnd` parses `editor-tab:*` prefix to route to detach actions. `layoutUtils.ts` helpers: `isDetachedEditor()`, `getDetachedTabId()`, `makeDetachedPanelId()`, `insertPanelAtNode/AtEdge/IntoNode` (insert without remove). localStorage key bumped to `nebulide-layout-v6`
- **Temp tabs (VS Code preview mode):** `workspaceStore.tempTabId` tracks the current "preview" tab. Single-click on file = opens/replaces temp tab (one at a time). Double-click / `inNewTab=true` = permanent tab. Editing content auto-pins the temp tab (`tempTabId = null` when `modified=true`). `pinTab(tabId)` explicitly pins. Temp tabs display in italic CSS (`.editor-tab.preview .truncate { font-style: italic }`). `closeTab`/`detachTab` clear `tempTabId` if it matches
- **Panel tab activation:** `toggleVisibility` in `layoutStore` uses `findPanelNode` + `setNodeActiveTab` to keep `node.activeIndex` in sync: on hide → activates nearest visible sibling; on show → activates the shown panel. Both use `setNodeActiveTab` from `layoutUtils.ts`
- **Sidebar:** Shows panel visibility toggle buttons (chat/files/editor/preview/terminal) instead of session list. Session management moved to ChatPanel. `SidebarToggleBtn` component in `DroppablePanel.tsx` appears in first panel's header (via `isFirst` prop) to toggle sidebar open/close. State: `workspaceStore.sidebarOpen` / `setSidebarOpen`
...
**Code-server proxy:** `GET|POST|PUT|... /code/*path` — requires JWT auth, proxies to `http://code-server:8443`
- Auth middleware supports `?token=` query param — used by WebSocket and iframe endpoints (PDF preview, code-server)
...
cd backend && go build -o nebulide .
...
- **Long-press on mobile:** `useLongPress` uses pointer events (`onPointerDown/Up/Move/Cancel`), NOT touch events. Ignores `pointerType === 'mouse'` (mouse has right-click). Tracks `activePointerRef` (pointerId). Use `mergeEventHandlers(longPressHandlers, listeners)` to combine with @dnd-kit listeners — if spread separately, latter overwrites former's `onPointerDown`. Guard onClick with `longPressedRef.current` to prevent click firing after long-press
- **Mobile touch event bubbling (legacy):** If using touch events: `touchstart` bubbles from child to parent. Fix: `e.stopPropagation()` in child. But prefer pointer events approach (useLongPress) which avoids this entirely
...
- **DnD touch drag offset:** `PointerSensor` replaced with `MouseSensor` + `TouchSensor`. `DragOverlay` uses `touchSnapCenter` modifier (Workspace.tsx) — snaps overlay center to finger position on touch, preventing visual offset jump that occurs when grab point ≠ element center
- **useChat stream batching:** `setStreamContent` is called on every WebSocket chunk (50-100/sec during streaming). Batched via `pendingDeltaRef` + `flushTimerRef` (50ms setTimeout). `scheduleDelta(text)` accumulates; `cancelFlush()` on `complete`/`error` clears timer+pending. Full content events (`assistant` type) call `cancelFlush()` then `setStreamContent(textBlocks)` directly
- **ContextMenu close inside Monaco/iframe:** Window `mousedown` listener never fires inside Monaco editor (it stopPropagation). Fix: `window.addEventListener('pointerdown', handleOutside, true)` — capture phase runs before any child stopPropagation
- **Windows go run workspace:** `os.Executable()` during `go run .` returns temp build path (`AppData/Local/go-build/...`). `os.Getwd()` also returns temp dir in some environments. Fix: `findProjectRoot()` in `config.go` walks up candidates from both `exe` and `getwd`, finds first dir containing `.env` = project root
